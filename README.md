...
### 方式三：BART（BRAT）安装（推荐给测试版用户）

1. 在社区插件市场安装并启用 **BRAT**（Beta Reviewer's Auto-update Tool）。
2. 打开 **Settings → BRAT**，点击 **Add Beta plugin**。
3. 输入仓库地址：`https://github.com/jiyee/obsidian-boox-plugin`（或 `jiyee/obsidian-boox-plugin`）。
4. 添加完成后，回到 **Settings → Community plugins** 启用 **Boox Highlights Sync**。
5. 后续可在 BRAT 中检查更新并安装新测试版。
